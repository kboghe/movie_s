...
######WELCOME MESSAGE#######
############################
print("Welcome to the Box Office scraper! \n \n This little script will fetch the Box Office info from one or multiple years and: \n (a) fetch the general user ratings on IMDB (weighted average, arithmetic mean, median and sample size of voters) \n (b) fetch the general (weighted) Metacritic rating from professional critics \n (c) fetch the user rating on IMDB for different socio-demographic groups (sex*age group + nationality) \n (d) fetch all individual reviews from professional critics featured on Metacritic (outlet,score) \n")
...
print("writing to Repl environment...")
boxoffice_mastertable.to_csv("boxoffice.csv")
print("done!\n")
#####################################################
...
for year in all_years: #This is a giant loop. 90% of the script is basically embededd within this loop. So, for all Box Office years we've scraped in the previous step, do the following:#
    ...
    #pause 30 minutes before next scraping session#
...
#############################################################
######### Writing dataframes to REPL environment ############
#############################################################

print("writing to Repl environment...")
general_user_ratings_imdb_final.to_csv("IMDB_general_ratings.csv")
print("done! \n")

print("writing to Repl environment...")
socio_demographic_user_rating_imdb_final.to_csv("IMDB_socio_demographic_ratings.csv")
print("done! \n")

print("writing to Repl environment...")
us_vs_other_users_rating_imdb_final.to_csv("IMDB_us_versus_others.csv")
print("done! \n")

print("writing to Repl environment...")
metacritic_reviews_final.to_csv("Metacritic_reviews.csv")
print("done! \n")

print()
print("writing Metacritic ids to Repl environment...\n")
metacritic_ids_list_final.to_csv("metacritic_ids.csv")
print("done! \n")

print("writing IMDB ids to Repl environment... \n")
imdb_ids_list_final.to_csv("imdb_ids.csv")
print("done! \n")